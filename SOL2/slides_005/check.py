"""Run: uv run --with pymupdf python SOL2/slides_005/check.py"""
import re
import subprocess
...
assert len(pdf) == 66, "Inspect slide breaks and answer reveals"
icon_sizes = {
...
assert tasks == list(range(1, 8)), "Task numbering changed"
assert code_rows > 150, "Code spacing check did not cover the examples"
text = " ".join(" ".join(page.get_text().split()) for page in pdf)
for label in ("~ name: String", "+ toString(): String", "Implicit super() fails to compile",
              "runtime type stays Pilot", "Fields are hidden, not overridden",
              "breathe()", "hashCode() too", "Assume Point overrides equals()"):
    assert label in text, ("Missing diagram content", label)
print(f"PASS: {len(pdf)} pages, 64 slides, 7 tasks, {code_rows} code-row gaps; bounds and vector diagrams checked")
