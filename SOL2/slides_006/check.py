"""Run: uv run --with pymupdf python SOL2/slides_006/check.py"""
import re
import subprocess
...
assert '[+ ' not in drawings, "Escape UML + signs to avoid Typst numbered lists"
...
assert len(pdf) == 43, "Inspect slide breaks and answer reveals"
icon_sizes = {
...
assert tasks == [1, 2, 3], "Task numbering changed"
assert code_rows >= 80, "Code spacing check did not cover the examples"
text = " ".join(" ".join(page.get_text().split()) for page in pdf)
for label in ("+ getNextImage(): Image", "+ resize(double): void", "Comparable<Shape>",
              "Abstract reference type", "Implicitly public static final",
              "One superclass", "zero does not mean object identity", "changed in place"):
    assert label in text, ("Missing diagram content", label)
print(f"PASS: {len(pdf)} pages, 40 slides, 3 tasks, {code_rows} code-row gaps; bounds and vector diagrams checked")
