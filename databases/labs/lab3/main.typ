...
#show: exercise.project.with(
  ...
  This is the third lab of Databases. This lab focuses on improving your skills in dealing with SQL queries. There are tasks for both DDL and DML statements.
  It is recommended that you familiarize yourself with the assignments to allow a more effective  participation in Laboratory 3.    If you have questions or need any support, help each other, or use the forum in our moodle room.  
...
Consider the following relational schema for the Student Information System (analogous to the Assignments from Lab 1):    
...
#figure(
table(columns: (auto, auto, auto, auto, auto), inset: 0.5em,
...
#figure(
table(columns: (auto, auto), inset: 0.5em,
...
3. Write a SQL-query for the created database that returns all students (first name + last  name) that study the program “Information Engineering”.  
...
#figure(image("../../assets/img/2024_11_18_lab3_employee_table_columns_rev01.png"))
...
4. Suppose that the EMPLOYEE table’s constraint EMPSUPERFK as specified in Figure 3 on the  next page is changed to read as follows:    
```sql
CONSTRAINT EMPSUPERFK  FOREIGN KEY (Super_ssn) REFERENCES EMPLOYEE(Ssn)  ON DELETE CASCADE ON UPDATE CASCADE;
```
Answer the following questions: 
- What happens when the following command is run on the database state shown in  Figure 2?
```sql
...
The following excerpt from a database schema models a database about a cinema. The following  assignments are to be answered in the form of relational algebra AND additionally in the form of  an SQL query.
...
2. Display a list of actors (first name, last name) and their roles in a specific movie (e.g.,  "FilmXYZ").
3. Find all movies that will be shown in the screenings (ScreeningID) for the movie theater  "HallA" on 2024-01-30 at 19:00.
4. Create a table with information about all reservations made by customers with the last  name "Schmidt", including the movie title and seat number.  
