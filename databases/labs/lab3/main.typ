...
*PLAYER*(PID, firstName, lastName, dateOfBirth)
*GAME*(GID, winnerID (FK), loserId (FK), remis)
The attributes winnerID and loserID are foreign keys to PID of  PLAYER. Specify the following natural language queries as relational algebra queries.
	1. Which chess players have won at least once? (Output: first name and last name)
...
= Assignment 6: Relational Algebra vs. SQL query for a Weather Database
